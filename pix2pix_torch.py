...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, drop_out=True):
        super(DecoderBlock, self).__init__()
        self.conv = nn.Conv2d(input_nc, output_nc, kernel_size=3, stride=1, padding='same')
        self.upsampling2d = nn.UpsamplingBilinear2d(scale_factor=2)
...
class UnetGenerator(nn.Module):
    def __init__(self):
        ...
        self.decoder5 = DecoderBlock(128+128, 64)
    ...
    def forward(self, x):
        # encoder forward:
        e1 = self.encoder1(x, norm_layer=False)
        e2 = self.encoder2(e1, norm_layer=True)
        e3 = self.encoder3(e2, norm_layer=True)
        e4 = self.encoder4(e3, norm_layer=True)
        e5 = self.encoder5(e4, norm_layer=True)
        e6 = self.relu1(self.encoder6(e5))
        # decoder forward with skip connections
        d1 = self.decoder1(e6, drop_out=False)
        d1 = torch.cat([d1, e5], dim=1)
        d2 = self.decoder2(d1, drop_out=False)
        d2 = torch.cat([d2, e4], dim=1)
        d3 = self.decoder3(d2, drop_out=False)
        d3 = torch.cat([d3, e3], dim=1)
        d4 = self.decoder4(d3, drop_out=False)
        d5 = torch.cat([d4, e2], dim=1)
        d6 = self.decoder5(d5, drop_out=False)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, norm_layer=False):
        if norm_layer:
            a = self.bn(self.conv(x))
        a = self.conv(x)
        return self.leakyrelu(a)
...
class Discriminator(nn.Module):
    ...
    def forward(self, output, labels):
        ...
        a = self.block1(x, norm_layer=True)
        a = self.block2(a, norm_layer=True)
        a = self.block3(a, norm_layer=True)
        a = self.block4(a, norm_layer=True)
